...
export function GuardrailLog({ fires = [], runId }: { fires: Fire[]; runId?: string }) {
  ...
  const hardBlocks = fires.filter(f => f.rule_level === "HARD_BLOCK").length;
  const softFlags = fires.filter(f => f.rule_level === "SOFT_FLAG").length;
  const autoActions = fires.filter(f => f.rule_level === "AUTO_ACTION").length;

  return (
    <div className="space-y-4 relative">
      {/* Management Statistics Banner */}
      <div className="grid grid-cols-3 gap-3">
        <div className="p-3 rounded-xl bg-red-500/10 border border-red-500/20 text-center">
          <p className="text-[10px] text-red-400 font-bold uppercase tracking-wider mb-1">Hard Blocks</p>
          <p className="text-xl font-bold text-white font-mono">{hardBlocks}</p>
        </div>
        <div className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/20 text-center">
          <p className="text-[10px] text-amber-400 font-bold uppercase tracking-wider mb-1">Soft Flags</p>
          <p className="text-xl font-bold text-white font-mono">{softFlags}</p>
        </div>
        <div className="p-3 rounded-xl bg-purple-500/10 border border-purple-500/20 text-center">
          <p className="text-[10px] text-purple-400 font-bold uppercase tracking-wider mb-1">Auto Actions</p>
          <p className="text-xl font-bold text-white font-mono">{autoActions}</p>
        </div>
      </div>
            ...
            <div className="grid grid-cols-2 gap-3 py-2 border-y border-white/5">
              {fire.vendor_name && (
                <div className="flex flex-col gap-0.5">
                  <span className="text-[10px] text-neutral-500 uppercase font-bold tracking-wider">Vendor / Entity</span>
                  <span className="text-xs text-neutral-300 truncate">🏢 {fire.vendor_name}</span>
                </div>
              )}
              {fire.amount_inr && fire.amount_inr > 0 && (
                <div className="flex flex-col gap-0.5">
                  <span className="text-[10px] text-neutral-500 uppercase font-bold tracking-wider">Financial Value</span>
                  <span className="text-xs text-neutral-300 font-mono">💰 {formatCurrency(fire.amount_inr)}</span>
                </div>
              )}
              <div className="col-span-2 flex flex-col gap-0.5">
                <span className="text-[10px] text-neutral-500 uppercase font-bold tracking-wider">Rule / Regulation</span>
                <span className="text-xs text-neutral-400 truncate">📋 {fire.regulation}</span>
              </div>
            </div>

            <div className="flex items-start justify-between gap-3 pt-1">
              <div className="space-y-1 flex-1">
                <span className="text-[10px] text-emerald-500 font-black uppercase tracking-widest">Management Recommendation</span>
                <div className="text-xs text-emerald-400 bg-emerald-500/10 rounded-lg px-3 py-2 border border-emerald-500/20">
                  ✅ {fire.action_taken}
                </div>
              </div>
